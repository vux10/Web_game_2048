...
function drawBoard() {
    ...
    ctx.fillRect(0, 0, WIDTH, HEIGHT - 100);  // Khu vực bảng 4x4
...
function move(direction) {
    ...
    if (direction === 'UP') {
        ...
        for (let col = 0; col < 4; col++) {
            ...
            // Cập nhật lại bảng sau khi di chuyển
            for (let row = 3; row >= 0; row--) {
                if (board[row][col] !== temp[3 - row]) {
                    moved = true;
                }
                board[row][col] = temp[3 - row] || 0;
            }
        }
        ...
        for (let row = 0; row < 4; row++) {
            ...
            // Cập nhật lại bảng sau khi di chuyển
            for (let col = 3; col >= 0; col--) {
                if (board[row][col] !== temp[3 - col]) {
                    moved = true;
                }
                board[row][col] = temp[3 - col] || 0;
            }
        }
    }
    return moved;
